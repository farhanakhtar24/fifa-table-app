    ...
    <div className="grid gap-8">
      <section>
        ...
        <div className="mt-4 grid gap-4">
          {upcoming.length === 0 ? (
            <p className="text-ice/60">Every fixture is in the book.</p>
          ) : (
  ...
  return (
    <Glass className="grid gap-4 p-5 md:grid-cols-[1fr_auto_1fr] md:items-center">
      ...
      <div className="flex items-center justify-center gap-3">
        ...
        <span className="font-display text-3xl text-ice/40">:</span>
  ...
  return (
    <div className={`flex items-center gap-3 ${align === "right" ? "flex-row-reverse text-right" : ""}`}>
      ...
      <div>
        ...
        <p className="hidden text-xs text-ice/50 sm:block">{team.name}</p>
      </div>
    </div>
  );
}
